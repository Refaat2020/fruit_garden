<div class="table-responsive animated fadeInRight" style="padding: 50px 30px 10px 30px;">
	...
	<?php if ( !empty( $approval ) && count( $approval->result()) > 0 ): ?>

		<?php foreach($approval->result() as $apv): ?>
			
			<tr>
				<td><?php echo ++$count;?></td>
				<td><?php echo $apv->name;?></td>
				<td><?php echo $apv->address1;?></td>
				<td><?php echo $apv->email;?></td>

				<?php if ( $this->ps_auth->has_access( EDIT )): ?>
			
					<td>
						<a href='<?php echo $module_site_url .'/edit/'. $apv->id; ?>'>
